...
$sql = "SELECT Produto, QuantidadeProduto, PreçoPro, ServiçoSolicitado, PreçoServ, Evento,  Localizacao, CPFcli,  StatusPedido FROM Vendas";
...
$pdf->Cell(40, 10, 'Serviço Solicitado', 1, 0, 'C');
$pdf->Cell(30, 10, 'Preço do Serviço', 1, 0, 'C');
$pdf->Cell(30, 10, 'Evento', 1, 0, 'C');
$pdf->Cell(30, 10, 'Produto', 1, 0, 'C');
$pdf->Cell(40, 10, 'Quantidade de Produto', 1, 0, 'C');
$pdf->Cell(30, 10, 'Preço do Produto', 1, 0, 'C');
$pdf->Cell(40, 10, 'Localização', 1, 0, 'C');
$pdf->Cell(30, 10, 'Status do Pedido', 1, 1, 'C');

// Dados do relatório
while ($row = $result->fetch_assoc()) {
    $pdf->Cell(30, 10, $row['CPFcli'], 1, 0, 'C');
    $pdf->Cell(40, 10, $row['ServiçoSolicitado'], 1, 0, 'C');
    $pdf->Cell(30, 10, 'R$ ' . number_format($row['PreçoServ'], 2, ',', '.'), 1, 0, 'C');
    $pdf->Cell(30, 10, $row['Evento'], 1, 0, 'C');
    $pdf->Cell(30, 10, $row['Produto'], 1, 0, 'C');
    $pdf->Cell(40, 10, $row['QuantidadeProduto'], 1, 0, 'C');
    $pdf->Cell(30, 10, 'R$ ' . number_format($row['PreçoPro'], 2, ',', '.'), 1, 0, 'C');
    $pdf->Cell(40, 10, $row['Localizacao'], 1, 0, 'C');
    $pdf->Cell(30, 10, $row['StatusPedido'], 1, 1, 'C');
}

// Saída do PDF
$pdf->Output('relatorio_vendas.pdf', 'D'); // 'D' força o download do arquivo PDF
